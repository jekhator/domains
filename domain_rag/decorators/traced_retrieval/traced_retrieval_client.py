...
def traced_retrieval(
    ...
    def decorator(target: Target) -> Target:
        ...
        if inspect.isclass(target):
            return cast(Target, _trace_class(target, principal_id, session_id, sink))
        elif callable(target):
            return cast(
                Target, _trace_callable(target, principal_id, session_id, sink)
            )
        else:
...
def _trace_callable(
    ...
    if asyncio.iscoroutinefunction(fn):
        return _wrap_async(fn, principal_id, session_id, sink)
    else:
        return _wrap_sync(fn, principal_id, session_id, sink)
